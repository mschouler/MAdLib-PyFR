...
LogSField* buildLogSField(pMesh &msh, double maxgrad, double maxani, double maxlength, double minlength) {
  ...
  */
  ...
  // high order length type
  logFld->setLengthType(MAd::LGT_QUADRATIC);
...
void combineMeshSol(std::string filename, std::string field, pMesh &msh, SolAtElements *sol, const double* geo) {
  /*
  ...
  */
  ...
  std::cout << "INFO -- number of cells: " << cells->GetNumberOfCells() << "\n";
  ...
  std::cout << "INFO -- number of components: " << array->GetNumberOfComponents() << "\n";
...
int main(int argc, char* argv[]) {
  ...
  int ord = msh->getOrder();
  if (ord < 2) {
    std::cout << "WARNING -- mesh should be of high order\n";
  }

  int nbrNod = ( ord + 1 ) * ( ord + 2 ) / 2;
  double *geo = new double[3*nbrNod];
  ...
  SolAtElements *sol = new SolAtElements(msh, logFld, 1, ord, geo, "Solution");
  std::cout << "DEBUG -- built solatelements\n";
  combineMeshSol(files[0], options["field"], msh, sol, geo);
  ...
  std::cout << "INFO -- sol order: " << sol->getOrder() << "\n";
  ...
  for (int ii=1; ii<nb_lines; ii++) {
    ...
    SolAtElements *tmpSol = new SolAtElements(tmpMsh, tmpLogFld, 1, ord, geo, "tmpSolution");
    combineMeshSol(files[ii], options["field"], tmpMsh, tmpSol, geo);
